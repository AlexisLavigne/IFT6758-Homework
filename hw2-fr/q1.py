...
with open("data/ontology.json", "r", encoding="utf-8") as f:
    ontology_map = {entry["id"]: entry["name"] for entry in json.load(f)}
...
def convert_ids(labels: str) -> str:
    ...
    """
    return "|".join(convert_id(label) for label in labels.split(','))
...
def contains_label(labels: pd.Series, label: str) -> pd.Series:
    ...
    "Music|Piano"
    """
    mask = labels.str.split('|').apply(lambda x: label in x)
    return labels[mask]
...
def get_correlation(labels: pd.Series, label_1: str, label_2: str) -> float:
    ...
    """
    contained_labels_1 = contains_label(labels, label_1)
    if contained_labels_1.empty:
        return 0.0
    contained_labels_2 = contains_label(contained_labels_1, label_2)
    return contained_labels_2.count() / contained_labels_1.count()
