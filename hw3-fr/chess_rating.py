...
def parse_xml(path: str) -> pd.DataFrame:
    ...
    for person in root:
        entry = {}
        for tag in TARGET_ATTRIBUTES:
            entry[tag] = _get_val(person, tag)
        data.append(entry)
...
def clean_data(df: pd.DataFrame, year_cutoff: int) -> pd.DataFrame:
    ...
    df['birthday'] = df['birthday'].astype(int) # car 'birthday' est juste une année et non une date complète (sinon j'aurais utilisé pd.to_datetime)
...
class PermutationTest:
    ...
    def job(self, seed: int = None) -> Tuple[int, int]:
        ...
        permuted_df = np.random.permutation(self.df['rating'])
        overrep_group = permuted_df[:self.n_overrep]
        underrep_group = permuted_df[self.n_overrep:self.n_overrep + self.n_underrep]
        max_overrep = np.max(overrep_group)
        max_underrep = np.max(underrep_group)
...
def sample_two_groups(
    ...
    for i in tqdm(range(n_iter)):
        max_overrep, max_underrep = PermutationTest(df, n_overrep, n_underrep).job()
