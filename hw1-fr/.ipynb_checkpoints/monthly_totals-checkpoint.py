...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    data['mois'] = data["date"].apply(date_to_month)

    grouped_precipitation_data = data.groupby(['name', 'mois'])['precipitation'].sum().reset_index()
    monthly = grouped_precipitation_data.pivot(index='name', columns='mois', values='precipitation')

    grouped_count_data = data.groupby(['name', 'mois'])['date'].count().reset_index()
    counts = grouped_count_data.pivot(index='name', columns='mois', values='date')
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    ```
    """
    distance_array = pdist(df.values, metric=func)
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    coords = df[['name', 'latitude', 'longitude']].drop_duplicates(subset=['name']).set_index('name')
    dist_matrix = compute_pairwise(coords, geodesic)
    return dist_matrix
...
def correlation(u, v) -> float:
    ...
    cov = ((u[idx] - mean_u) * (v[idx] - mean_v)).mean()
    corr = cov / (std_u * std_v)
...
def compute_pairwise_correlation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dist_matrix = compute_pairwise(df.T, correlation)
    return dist_matrix
